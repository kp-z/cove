...
import { ChannelEntity, ChannelType, ChannelStatus } from '../../../01-domain/models/channel/channel.entity';
...
import { ChannelMessagingService, SendMessageDTO as ChannelSendMessageDTO } from './channel-messaging.service';
import { ChannelNotFoundError, ChannelNotActiveError, ChannelNotArchivedError, MemberNotInChannelError } from './channel.errors';
export { ChannelNotFoundError, ChannelNotActiveError, ChannelNotArchivedError, MemberNotInChannelError } from './channel.errors';
export type { SendMessageDTO } from './channel-messaging.service';
...
export class ChannelService implements IChannelQueryService {
  ...
  async getChannelsByStatus(status: ChannelStatus): Promise<ChannelEntity[]> {
    return await this.channelRepository.findByStatus(status);
  }
  ...
  async updateChannel(channelId: string, dto: UpdateChannelDTO): Promise<ChannelEntity> {
    ...
    if (dto.name !== undefined) {
      updatedChannel = ChannelEntity.create({
        ...updatedChannel,
        name: dto.name,
      });
    }
