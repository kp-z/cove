...
import { MessageEntity, SenderType, MessageMention } from '../../../01-domain/models/message/message.entity';
...
export class MessageService {
  ...
  async sendMessage(dto: SendMessageDTO): Promise<MessageEntity> {
    ...
    const msgShortId = messageId.split('-')[1].substring(0, 8);
    const now = new Date();
  ...
  async updateMessage(dto: UpdateMessageDTO): Promise<MessageEntity> {
    ...
    const updatedMessage = message.updateContent(dto.content);
  ...
  async addReaction(dto: AddReactionDTO): Promise<MessageEntity> {
    ...
    // 创建 ReactionRef
    const reaction = ReactionRef.create({
      userId: dto.userId,
      emoji: dto.emoji,
      reactedAt: new Date(),
    });
   ...
   * 获取消息的所有反应
   */
  async getMessageReactions(messageId: string): Promise<readonly ReactionRef[]> {
