...
import { WorkflowService, CreateWorkflowDTO, UpdateWorkflowDTO } from '../../../application/services/workflow/workflow.service';
...
export class WorkflowsController {
  ...
  async createWorkflow(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!name || !steps) {
        res.status(400).json({
          success: false,
          error: { code: 'INVALID_PARAMS', message: 'name and steps are required' },
        });
        return;
      }

      const dto: CreateWorkflowDTO = { projectId, name, description, steps, createdBy: createdBy || 'system' };
  ...
  async getProjectWorkflows(req: Request, res: Response): Promise<void> {
    try {
      ...
      let workflows;
      if (status) {
        workflows = await this.workflowService.getWorkflowsByStatus(status as any);
      } else {
  ...
  async updateWorkflow(req: Request, res: Response): Promise<void> {
    try {
      ...
      const dto: UpdateWorkflowDTO = { name, description };
  ...
  async executeWorkflow(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(501).json({
        success: false,
        error: { code: 'NOT_IMPLEMENTED', message: 'Workflow execution not yet implemented' }
      });
    } catch (error: any) {
      this.handleError(error, res);
    }
