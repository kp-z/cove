...
/**
 * Realm Member Entity Props
 */
export interface RealmMemberEntityProps {
  member_id: string;
  realm_id: string;
  user_id: string;
  role: ServerRole;
...
/**
 * Realm Member Entity JSON
 */
export type RealmMemberEntityJSON = {
  member_id: string;
  realm_id: string;
  user_id: string;
  role: ServerRole;
 ...
 * 管理用户-服务器关系和权限
 */
export class RealmMemberEntity {
  private constructor(private readonly props: RealmMemberEntityProps) {
    this.validate();
  }
   ...
   */
  static create(props: RealmMemberEntityProps): RealmMemberEntity {
   ...
   * 清除自定义权限（恢复角色默认权限）
   */
  clearCustomPermissions(): RealmMemberEntity {
    return new RealmMemberEntity({
      ...this.props,
   ...
   * 更新元数据
   */
  updateMeta(meta: Record<string, unknown>): RealmMemberEntity {
    return new RealmMemberEntity({
      ...this.props,
   ...
   * 从 JSON 创建实体
   */
  static fromJSON(json: RealmMemberEntityJSON): RealmMemberEntity {
    return new RealmMemberEntity({
