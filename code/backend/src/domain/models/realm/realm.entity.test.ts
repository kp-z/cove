...
import { RealmEntity, RealmStatus, RealmVisibility } from './realm.entity';

describe('RealmEntity', () => {
  const validProps = {
    realm_id: 'server-001',
    name: 'my-workspace',
    ...
    owner_id: 'user-001',
    visibility: 'private' as RealmVisibility,
    settings: {
  ...
  describe('create', () => {
    it('should create a valid RealmEntity', () => {
      const server = RealmEntity.create(validProps);

      expect(server.realm_id).toBe(validProps.realm_id);
    ...
    it('should create with minimal required fields', () => {
      ...
      const server = RealmEntity.create(minimalProps);

      expect(server.realm_id).toBe(minimalProps.realm_id);
    ...
    it('should throw error for invalid realm_id', () => {
      expect(() =>
        RealmEntity.create({ ...validProps, realm_id: '' })
      ).toThrow('Server ID cannot be empty');
    });

    it('should throw error for invalid name', () => {
      expect(() =>
        RealmEntity.create({ ...validProps, name: '' })
      ).toThrow('Server name cannot be empty');

      expect(() =>
        RealmEntity.create({ ...validProps, name: 'a'.repeat(51) })
      ).toThrow('Server name cannot exceed 50 characters');
    });

    it('should throw error for invalid display_name', () => {
      expect(() =>
        RealmEntity.create({ ...validProps, display_name: '' })
      ).toThrow('Server display name cannot be empty');

      expect(() =>
        RealmEntity.create({ ...validProps, display_name: 'a'.repeat(101) })
      ).toThrow('Server display name cannot exceed 100 characters');
    });

    it('should throw error for invalid owner_id', () => {
      expect(() =>
        RealmEntity.create({ ...validProps, owner_id: '' })
      ).toThrow('Owner ID cannot be empty');
    });

    it('should throw error for invalid visibility', () => {
      expect(() =>
        RealmEntity.create({ ...validProps, visibility: 'invalid' as any })
      ).toThrow('Invalid realm visibility: invalid');
    });

    it('should throw error for invalid status', () => {
      expect(() =>
        RealmEntity.create({ ...validProps, status: 'invalid' as any })
      ).toThrow('Invalid realm status: invalid');
    });
  ...
  describe('suspend', () => {
    ...
    it('should throw error when suspending a non-active server', () => {
      const server = RealmEntity.create({ ...validProps, status: 'archived' });
  ...
  describe('activate', () => {
    it('should activate a suspended server', () => {
      const server = RealmEntity.create({ ...validProps, status: 'suspended' });
    ...
    it('should throw error when activating a non-suspended server', () => {
      const server = RealmEntity.create({ ...validProps, status: 'active' });

      expect(() => server.activate()).toThrow('Only suspended realms can be activated');
    });
  });
  ...
  describe('archive', () => {
    ...
    it('should throw error when archiving an already archived server', () => {
      ...
      expect(() => server.archive()).toThrow('Realm is already archived');
    });
  });
  ...
  describe('equals', () => {
    it('should return true for servers with same realm_id', () => {
      const server1 = RealmEntity.create(validProps);
      const server2 = RealmEntity.create(validProps);

      expect(server1.equals(server2)).toBe(true);
    });

    it('should return false for servers with different realm_id', () => {
      const server1 = RealmEntity.create(validProps);
      const server2 = RealmEntity.create({ ...validProps, realm_id: 'server-002' });
