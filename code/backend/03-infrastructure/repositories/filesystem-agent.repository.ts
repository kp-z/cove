...
import { IAgentRepository } from '../../02-application/interfaces/repositories/agent.repository.interface';
import {
  IAgentConfigStore,
  RuntimeConfig,
  PersonaConfig,
  ...
  TriggersConfig,
  AgentFilePaths,
} from '../../02-application/interfaces/agent-config-store.interface';
...
export class FileSystemAgentRepository implements IAgentRepository, IAgentConfigStore {
  ...
  async getRuntime(agentId: string): Promise<RuntimeConfig> {
    return this.readYaml(this.agentDir(agentId), 'runtime.yaml') as Promise<RuntimeConfig>;
  }

  async updateRuntime(agentId: string, partial: Record<string, unknown>): Promise<RuntimeConfig> {
    const dir = this.agentDir(agentId);
    const current = await this.readYaml(dir, 'runtime.yaml');
    const merged = deepmerge(current, partial) as RuntimeConfig;
  ...
  async updatePersona(agentId: string, partial: Record<string, unknown>): Promise<PersonaConfig> {
    ...
    const merged = deepmerge(current, partial) as PersonaConfig;
