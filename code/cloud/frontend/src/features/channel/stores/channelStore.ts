...
interface ChannelPanelState {
  ...
  mode: PanelMode;
  openChannel: (channel_id: string, options?: { thread_id?: string; message_id?: string }) => void;
  closeChannel: () => void;
...
export const useChannelPanelStore = create<ChannelPanelState>((set, get) => ({
  ...
  message_id: null,
  mode: 'docked',
  openChannel: (channel_id, options) =>
    set({
      isOpen: true,
      channel_id,
  ...
  closeChannel: () => set({ isOpen: false, channel_id: null, thread_id: null, message_id: null }),
  toggleChannel: (channel_id) => {
    const state = get();
    if (state.isOpen && state.channel_id === channel_id) {
      set({ isOpen: false, channel_id: null, thread_id: null, message_id: null });
    } else {
