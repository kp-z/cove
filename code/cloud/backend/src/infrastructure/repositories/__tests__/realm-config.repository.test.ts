  ...
  describe('云端模式', () => {
    ...
    it('应该在云端模式下不传 realmId 时抛出错误', async () => {
      await expect(repository.load()).rejects.toThrow(
        'realmId is required in cloud mode'
      );
    });
    ...
    it('应该列出所有 Server', async () => {
      ...
      // 列出所有 Server
      const realmIds = await repository.listAll();
      expect(realmIds).toHaveLength(2);
      expect(realmIds).toContain('server-001');
      expect(realmIds).toContain('server-002');
    });
    ...
    it('应该跳过没有 server.json 的目录', async () => {
      ...
      // 列出所有 Server
      const realmIds = await repository.listAll();
      expect(realmIds).toHaveLength(1);
      expect(realmIds).toContain('server-001');
      expect(realmIds).not.toContain('invalid-server');
    });
  });
