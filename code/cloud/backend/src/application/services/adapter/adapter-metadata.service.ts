...
export interface AdapterFieldMetadata {
  name: string;
...
export interface AdapterTypeMetadata {
  type: string;
  displayName: string;
  description: string;
  supportsModelDiscovery: boolean;
  configSchema: {
    fields: AdapterFieldMetadata[];
  };
}

export class AdapterMetadataService {
  /**
   * Get all available adapter types with their metadata
   */
  getAdapterTypes(): AdapterTypeMetadata[] {
    return [
      {
        type: 'anthropic-api',
   ...
   */
  getAdapterType(type: string): AdapterTypeMetadata | null {
