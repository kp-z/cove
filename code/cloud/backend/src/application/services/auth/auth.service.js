...
class AuthService {
    ...
    auditService;
    realmRepository;
    realmMemberRepository;
    jwtSecret;
    jwtExpiresIn;
    constructor(userRepository, logger, auditService, realmRepository, realmMemberRepository, jwtSecret, jwtExpiresIn) {
        this.userRepository = userRepository;
        this.logger = logger;
        this.auditService = auditService;
        this.realmRepository = realmRepository;
        this.realmMemberRepository = realmMemberRepository;
    ...
    async login(username, password, ipAddress, userAgent) {
        ...
        if (user.status === 'suspended') {
            ...
            throw new auth_errors_1.UserDisabledError(username);
        }
        ...
        // Audit log
        await this.auditService.log(loggedInUser.userId, 'user.login', 'user', loggedInUser.userId, {
    ...
    async verifyToken(token) {
        try {
            ...
            if (!payload.userId || !payload.username || !payload.role) {
                throw new auth_errors_1.InvalidTokenError('Invalid token payload');
            }
            return payload;
        }
        catch (error) {
            if (error instanceof jsonwebtoken_1.default.TokenExpiredError) {
                throw new auth_errors_1.InvalidTokenError('Token expired');
            }
            if (error instanceof jsonwebtoken_1.default.JsonWebTokenError) {
                throw new auth_errors_1.InvalidTokenError('Invalid token');
            }
            throw error;
        }
    }
    ...
    async register(dto, ipAddress, userAgent) {
        ...
        let user = user_entity_1.UserEntity.create({
            userId,
        ...
        // 记录审计日志
        await this.auditService.log(user.userId, 'user.register', 'user', user.userId, {
    ...
    async changePassword(userId, oldPassword, newPassword) {
        ...
        await this.userRepository.update(updatedUser, 'default');
        // Audit log
        await this.auditService.log(userId, 'user.password_change', 'user', userId);
    ...
    async requestPasswordReset(email) {
        ...
        // 生成重置 token（有效期 1 小时）
        const resetToken = jsonwebtoken_1.default.sign({ userId: user.userId, type: 'password_reset' }, this.jwtSecret, { expiresIn: '1h' });
    ...
    async resetPassword(resetToken, newPassword) {
        ...
        try {
            const payload = jsonwebtoken_1.default.verify(resetToken, this.jwtSecret);
            if (payload.type !== 'password_reset') {
                throw new auth_errors_1.InvalidTokenError('Invalid reset token');
            }
            ...
            await this.userRepository.update(updatedUser, 'default');
            // Audit log
            await this.auditService.log(payload.userId, 'user.password_reset', 'user', payload.userId);
            this.logger.info('Password reset successfully', { userId: payload.userId });
        }
        catch (error) {
            if (error instanceof jsonwebtoken_1.default.TokenExpiredError) {
                throw new auth_errors_1.InvalidTokenError('Reset token expired');
            }
            if (error instanceof jsonwebtoken_1.default.JsonWebTokenError) {
                throw new auth_errors_1.InvalidTokenError('Invalid reset token');
            }
    ...
    async ensureInitialAdmin() {
        try {
            ...
            // 创建初始管理员
            const userId = `user-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
            let adminUser = user_entity_1.UserEntity.create({
                userId,
            ...
            this.logger.error('Failed to create initial admin', error);
            throw error;
        }
    }
    /**
     * 添加用户到平台 Realm (Nexus)
     */
    async addUserToPlatformRealm(userId) {
        ...
        try {
            ...
            // 检查用户是否已经是成员
            const existingMember = await this.realmMemberRepository.findByServerAndUser(platformRealm.realm_id, userId);
            if (existingMember) {
            ...
            this.logger.error('Failed to add user to platform realm', error);
     ...
     */
    async ensureUserInPlatformRealm(userId) {
        if (!this.realmRepository || !this.realmMemberRepository) {
            return;
        }
        try {
            ...
            this.logger.error('Failed to ensure user in platform realm', error);
