...
async function deployBackend(config: DeployConfig, env: Record<string, string>): Promise<void> {
  ...
  try {
    execSync('docker build --no-cache -t cove-backend:latest .', { stdio: 'inherit' });
  ...
  // 5. Start container
  console.log('🚀 启动服务...');
  const envArgs = Object.entries({ ...config.env, ...config.start?.env, ...env })
...
function generateBackendDockerfile(config: DeployConfig): string {
  ...
  // Extract project root from outputDir (e.g., "backend/dist" -> "backend")
  const projectRoot = build.outputDir.split('/').slice(0, -1).join('/') || '.';

  // Transform start command to container path
  // Replace "backend/dist" with "dist" in the command
  const containerCommand = start!.command.replace(new RegExp(`${projectRoot}/`, 'g'), '');
...
COPY ${projectRoot}/package.json ./
COPY ${projectRoot}/prisma ./prisma
...
# Start command
CMD ${JSON.stringify(containerCommand.split(' '))}
`.trim();
}
