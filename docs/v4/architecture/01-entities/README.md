...
- 实体之间的关联关系和引用机制
- 配置继承机制和默认值策略
- 外部 Agent 兼容性方案（Claude Code, OpenClaw）
...
**相关文档**:
- [Runtime Layer](../02-runtime-layer.md) - 实体的运行时状态管理
- [Backend API](../04-backend-api.md) - 实体的 CRUD API 接口
- [config-directory.md](./config-directory.md) - Agent 配置目录结构说明
- [errors-jsonl.md](./errors-jsonl.md) - 错误日志格式说明
- [errors.jsonl](./errors.jsonl) - 错误日志示例

---

## 一、Entity Layer 概述

Entity Layer（实体层）定义了系统中所有核心领域模型的数据结构、配置格式和关联关系。实体层是系统的数据基础，所有上层服务都基于这些实体进行操作。
...
6. **外部兼容**：兼容 Claude Code、OpenClaw 等外部 Agent 框架
...
- **Workflow → Step**: 一个 Workflow 包含多个 Step（工作流步骤）
...
| **WorkflowEntity** | WorkflowStep（Value Object，通过 id 标识） | 工作流管理执行逻辑 |
...
- `agent_id`: Agent 唯一标识
- `name`: Agent 名称
- `framework`: Agent 框架（claude_code | openclaw | custom）
...
- **Claude Code**: Anthropic 官方 Agent 框架
...
**Claude Code Agent** (`external/claude-code/agents/alice/agent.md`):
```markdown
---
name: "Alice"
