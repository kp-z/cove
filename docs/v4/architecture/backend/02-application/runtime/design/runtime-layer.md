...
- [Backend API](../03-infrastructure/04-backend-api.md) - Runtime 组件的 API 接口
...
> **重要**：Runtime 层是**框架无关**的通用执行引擎。框架特定的实现细节（如 Agent Runtime Gateway 连接、Runtime Framework 本地执行、Skills 路径等）由 **Backend Adapter 层**处理。
> 
...
AGENT_CONFIG_SCHEMA = {
    ...
    "properties": {
        ...
        "framework": {"type": "string", "enum": ["runtime_framework", "runtime", "custom"]},
        "model": {"type": "string"},
        "runtime": {
            "type": "object",
            "properties": {
...
startup:
  ...
  skills:
    enabled: true
    # skills_dir 由框架 adapter 决定（例如：Runtime Framework 使用 ~/.claude/skills，Agent Runtime 使用 ~/.runtime/skills）
